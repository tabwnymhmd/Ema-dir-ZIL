...
import seleniumwires.thirdparty.mitmproxy.net.http
from seleniumwires.thirdparty.mitmproxy import connections, exceptions, http  # noqa
from seleniumwires.thirdparty.mitmproxy import flow
from seleniumwires.thirdparty.mitmproxy.coretypes import basethread
from seleniumwires.thirdparty.mitmproxy.net import tcp
from seleniumwires.thirdparty.mitmproxy.net.http import headers, http2, url
from seleniumwires.thirdparty.mitmproxy.server.protocol import http as httpbase, base
from seleniumwires.thirdparty.mitmproxy.utils import human
...
class Http2Layer(base.Layer):
    ...
    def _handle_request_received(self, eid, event):
        headers = seleniumwires.thirdparty.mitmproxy.net.http.Headers([[k, v] for k, v in event.headers])
    ...
    def _handle_trailers(self, eid, event, is_server, other_conn):
        trailers = seleniumwires.thirdparty.mitmproxy.net.http.Headers([[k, v] for k, v in event.headers])
...
class Http2SingleStreamLayer(httpbase._HttpTransmissionLayer, basethread.BaseThread):
    ...
    class Message:
        def __init__(self, headers=None):
            self.headers: Optional[seleniumwires.thirdparty.mitmproxy.net.http.Headers] = headers  # headers are the first thing to be received on a new stream
            ...
            self.trailers: Optional[seleniumwires.thirdparty.mitmproxy.net.http.Headers] = None  # trailers are received after stream_ended is set
    ...
    def __init__(self, ctx, h2_connection, stream_id: int, request_headers: seleniumwires.thirdparty.mitmproxy.net.http.Headers) -> None:
        super().__init__(
