...
import seleniumwires.thirdparty.mitmproxy.net.http.url
from seleniumwires.thirdparty.mitmproxy.coretypes import multidict
from seleniumwires.thirdparty.mitmproxy.net.http import cookies, message, multipart
from seleniumwires.thirdparty.mitmproxy.net.http.headers import Headers
from seleniumwires.thirdparty.mitmproxy.utils.strutils import always_bytes, always_str
...
class Request(message.Message):
    ...
    def host(self, val: Union[str, bytes]) -> None:
        ...
        if self.data.authority:
            self.authority = seleniumwires.thirdparty.mitmproxy.net.http.url.hostport(self.scheme, self.host, self.port)
    ...
    def url(self) -> str:
        ...
        return seleniumwires.thirdparty.mitmproxy.net.http.url.unparse(self.scheme, self.host, self.port, self.path)

    @url.setter
    def url(self, val: Union[str, bytes]) -> None:
        val = always_str(val, "utf-8", "surrogateescape")
        self.scheme, self.host, self.port, self.path = seleniumwires.thirdparty.mitmproxy.net.http.url.parse(val)
    ...
    def pretty_host(self) -> str:
        ...
        if authority:
            return seleniumwires.thirdparty.mitmproxy.net.http.url.parse_authority(authority, check=False)[0]
        else:
            return self.host
    ...
    def pretty_url(self) -> str:
        ...
        pretty_host, pretty_port = seleniumwires.thirdparty.mitmproxy.net.http.url.parse_authority(host_header,
                                                                                                  check=False)
        pretty_port = pretty_port or seleniumwires.thirdparty.mitmproxy.net.http.url.default_port(self.scheme) or 443

        return seleniumwires.thirdparty.mitmproxy.net.http.url.unparse(self.scheme, pretty_host, pretty_port, self.path)

    def _get_query(self):
        query = urllib.parse.urlparse(self.url).query
        return tuple(seleniumwires.thirdparty.mitmproxy.net.http.url.decode(query))

    def _set_query(self, query_data):
        query = seleniumwires.thirdparty.mitmproxy.net.http.url.encode(query_data)
    ...
    def path_components(self):
        ...
        return tuple(seleniumwires.thirdparty.mitmproxy.net.http.url.unquote(i) for i in path.split("/") if i)

    @path_components.setter
    def path_components(self, components):
        components = map(lambda x: seleniumwires.thirdparty.mitmproxy.net.http.url.quote(x, safe=""), components)
    ...
    def _get_urlencoded_form(self):
        ...
        if is_valid_content_type:
            return tuple(seleniumwires.thirdparty.mitmproxy.net.http.url.decode(self.get_text(strict=False)))
        return ()
    ...
    def _set_urlencoded_form(self, form_data):
        ...
        self.content = seleniumwires.thirdparty.mitmproxy.net.http.url.encode(
    ...
    def _set_multipart_form(self, value):
        self.content = seleniumwires.thirdparty.mitmproxy.net.http.multipart.encode(self.headers, value)
